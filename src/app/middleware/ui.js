...
const updateUI = async () => {
    try {
        ...
        if (isAuthenticated) {
            ...
            document.querySelector('.user-avatar').src = user.picture;
            document.querySelector('#user-name').innerText = user.name;

            // eachElement(".user-name", (e) => (e.innerText = user.name));
            // eachElement(".user-email", (e) => (e.innerText = user.email));
            document.querySelector('.auth-visible').classList.remove('d-none');
            document.querySelector('.auth-invisible').classList.add('d-none');
            // eachElement(".auth-invisible", (e) => e.classList.add("hidden"));
            // eachElement(".auth-visible", (e) => e.classList.remove("hidden"));
        } else {
            document.querySelector('.auth-visible').classList.add('d-none');
            document
                .querySelector('.auth-invisible')
